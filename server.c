          ...
          case 'd':
            ...
            printf("Performing Deposit\n");
             ...
             // sleep(100);
              ...
              printf("Account balance after withdrawal is: %d\n",bank_accounts[i].balance_amount);
              // strcpy(message,strcat("Transaction successful. Account balance after withdrawal is:",itoa(bank_accounts[i].balance_amount)));
...
void *transaction_processor(void *socket_desc)
{
   ...
   while((read_size = recv(sock,client_message,2000,0))>0){
    printf("Transaction request from client %s\n",client_message );
